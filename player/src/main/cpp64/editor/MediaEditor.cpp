...
static FilteringContext1 *filter_ctx;
static StreamContext1 *stream_ctx;
...
static int open_input_file(const char *filename) {
    ...
    stream_ctx = static_cast<StreamContext1 *>(av_mallocz_array(ifmt_ctx->nb_streams,
                                                                sizeof(*stream_ctx)));
    if (!stream_ctx)
...
static int init_filter(FilteringContext1 *fctx, AVCodecContext *dec_ctx,
...
static int init_filters(void) {
    ...
    unsigned int i;
    int ret;
    filter_ctx = static_cast<FilteringContext1 *>(av_malloc_array(ifmt_ctx->nb_streams,
                                                                  sizeof(*filter_ctx)));
    if (!filter_ctx)
        return AVERROR(ENOMEM);
...
static int encode_write_frame(AVFrame *filt_frame, unsigned int stream_index) {
    ...
    while (1) {
        ...
        if (ret == AVERROR(EAGAIN) || ret == AVERROR_EOF) {
            ...
            LOGI("Muxing frame, enc_pkt->dts=%ld, enc_pkt->pts=%ld,"\
                   " enc_ctx->time_base=%d/%d, ofmt_ctx->time_base=%d/%d, stream_index=%s\n",
                 enc_pkt.dts, enc_pkt.pts,
                 enc_timebase.num, enc_timebase.den, ofmt_timebase.num, ofmt_timebase.den,
                 stream_ctx[stream_index].enc_ctx->codec_type == AVMEDIA_TYPE_AUDIO ? "audio"
                                                                                    : "video");
            /* mux encoded frame */
...
static int encode_write_frame_fifo(AVFrame *filt_frame, unsigned int stream_index) {
    ...
    if (stream_ctx[stream_index].enc_ctx->codec_type == AVMEDIA_TYPE_AUDIO) {
         ...
         * the encoder. */
        while (av_audio_fifo_size(stream_ctx[stream_index].fifo) >=
            ...
            const int frame_size = FFMIN(av_audio_fifo_size(stream_ctx[stream_index].fifo),
                                         stream_ctx[stream_index].enc_ctx->frame_size);

            AVFrame *output_frame;
            /* Initialize temporary storage for one output frame. */
            if (init_output_frame(&output_frame, stream_ctx[stream_index].enc_ctx, frame_size) <
                0) {
            ...
            if (av_audio_fifo_read(stream_ctx[stream_index].fifo, (void **) output_frame->data,
                                   frame_size) < frame_size) {
...
MediaEditor::MediaEditor(const char *input, const char *output) {
    this->inputFile = input;
    this->outputFile = output;
}

int MediaEditor::start() {
    int ret;
    ...
    if ((ret = open_input_file(this->inputFile)) < 0)
        goto end;
    if ((ret = open_output_file(this->outputFile)) < 0)
        goto end;
