package com.simple.rtmp.encoder;
...
import com.simple.rtmp.egl.EglHelper;
import com.simple.rtmp.egl.EglSurfaceView;
import com.simple.rtmp.util.ByteUtil;
...
public abstract class BasePushEncoder {
    ...
    private void initVideoEncoder(String mineType, int width, int height) {
        try {
            Log.d("rtmp-log", "width:" + width + "  height: " + height);
            if (width % 2 != 0) {
                width--;
            }
            if (height % 2 != 0) {
            ...
            while (true) {
                ...
                if (outputBufferIndex == MediaCodec.INFO_OUTPUT_FORMAT_CHANGED) {
                    ...
                    while (outputBufferIndex >= 0) {
                        ...
                        //                        Log.e("zzz", "VideoTime = " + videoBufferinfo.presentationTimeUs /
                        //                        1000000.0f);

                        if (encoderWeakReference.get().onMediaInfoListener != null) {
                            encoderWeakReference.get().onMediaInfoListener.onSPSPPSInfo(encoderWeakReference.get().sps, encoderWeakReference.get().pps);
                        }
    ...
    static class EGLMediaThread extends Thread {
        ...
        private void onCreate() {
            if (!isCreate || encoderWeakReference.get().mRender == null) return;
        ...
        private void onChange(int width, int height) {
            if (!isChange || encoderWeakReference.get().mRender == null) return;
        ...
        private void onDraw() {
            if (encoderWeakReference.get().mRender == null) return;
